...
function toDeliver($id)
{
    ...
    if ($rider_result->num_rows > 0) {
        ...
        if ($rider['EMP_TYPE'] === 'Rider') {
            ...
            $order_sql = "SELECT * FROM `order` WHERE RIDER_ID = '$id' AND DELIVERY_TYPE = 'Deliver' AND STATUS = 'For-Delivery'";
            ...
            if($order_result->num_rows > 0){
                ...
                while($order = $order_result->fetch_assoc()){
                    ...
                    $order_details_result = $conn->query("SELECT * FROM order_details WHERE TRANSACTION_ID = '$transaction_id'");
                    $order_details_array = [];
