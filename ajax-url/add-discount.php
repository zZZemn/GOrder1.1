...
if (isset($_SESSION['id'])) {
    ...
    if (isset($_POST['discountName']) && isset($_POST['discountPercentage'])) {
        ...
        $check_disc_name = "SELECT * FROM discount WHERE DISCOUNT_NAME = '$discount_name'";
        $check_disc_result = $conn->query($check_disc_name);
        if ($check_disc_result->num_rows > 0) {
            echo "exisiting";
        } else {
            ...
            if ($conn->query($insert_discount_sql) === TRUE && $conn->query($add_discount_log) === TRUE) {
                ...
                // echo "invalid 1".$discount_percentage;
